...
Sub RenderImageBinary()
    ...
    Dim img, bytes, ContentType, stampText
    ...
    img.DrawRectangle 20, 20, 600, 220
    img.Fill

    img.SetHexColor "#ffffff"
    img.DrawCircle 110, 130, 64
    img.Fill

    img.SetHexColor "#0f172a"
    img.DrawStringAnchored "G3IMAGE runtime test", 360, 104, 0.5, 0.5
    img.DrawStringAnchored "Dynamic date/time image", 360, 132, 0.5, 0.5

    stampText = "Generated: " & CStr(Now())
    img.DrawStringAnchored stampText, 360, 166, 0.5, 0.5
...
<html lang="en">
    ...
    <body>
        ...
        <div class="box">
            ...
            <p>
                HasContext before Close:
                <code><%= beforeClose %></code>
            </p>
            <p>
                HasContext after Close:
                <code><%= afterClose %></code>
            </p>
            <p>
                LastMimeType before Close:
                <code><%= Server.HTMLEncode(beforeMime) %></code>
            </p>
            <p>
                LastMimeType after Close:
                <code><%= Server.HTMLEncode(afterMime) %></code>
            </p>
<%
    If LCase(beforeClose) = "true" And LCase(afterClose) = "false" Then
%>
            <p class="ok">
                <strong>PASS:</strong> Context reference was released.
            </p>
<%
    Else
%>
            <p class="bad">
                <strong>FAIL:</strong> Context was not released as expected.
            </p>
<%
    End If
%>
        ...
        <div class="box">
            ...
            <p>
                This image is generated by <code>G3IMAGE</code> and streamed via
                <code>Response.BinaryWrite</code>.
            </p>
            <img
                src="test_g3image.asp?mode=image&ts=<%= Timer() %>"
                alt="G3IMAGE output"
            />
        </div>
