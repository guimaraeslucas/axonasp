# Program Classic ASP with LLM/AI agents in AxonASP

## Overview
Use this guide when asking LLM/AI agents to generate or refactor Classic ASP VBScript code for AxonASP. The objective is strict compatibility, predictable runtime behavior, and safe code generation aligned with project standards.

## Syntax
```text
Task for AI agent:
- Target runtime: Classic ASP VBScript 5.8
- Enforce: block If...Then...End If only
- Enforce: Set for object assignment
- Enforce: & for string concatenation
- Enforce: Option Explicit
- Enforce: no inline Dim initialization
```

## Parameters and Arguments
- Runtime target (Required): Classic ASP with VBScript 5.8 semantics.
- Directive format (Required): Keep <%@ ... %> on the first line, compact, and fully closed.
- Control flow policy (Required): Use block If...Then...End If only. Do not use single-line If statements.
- Variable policy (Required): Use Option Explicit and separate declaration from assignment.
- Object policy (Required): Use Set for object references and Set obj = Nothing for cleanup.
- Call syntax policy (Required): For Sub/method calls without return, use no parentheses or use Call with parentheses.
- Function call policy (Required): Use parentheses when assigning function return values.
- Error policy (Required): Use On Error Resume Next, check Err.Number, and reset with On Error GoTo 0.
- Boolean policy (Required): Avoid short-circuit assumptions for And and Or; nest conditions where safety is required.
- Concatenation policy (Required): Use & for strings. Do not use + for string concatenation.
- Object creation policy (Required): Use exact ProgID values for Server.CreateObject.

## Return Values
Using these constraints in prompts produces code that is more likely to execute without parser or runtime regressions in Classic ASP and AxonASP.

## Remarks
- Treat every generated snippet as compatibility-sensitive code.
- Ask the LLM to avoid modern language patterns such as try/catch, typed declarations, or inline initialization.
- Require explicit object lifecycle cleanup in long-running scripts.
- Require defensive condition nesting for recordset and object access.
- Validate generated ProgIDs against the AxonASP manual before execution.

## Agent Prompting Example
When asking an AI agent to generate Classic ASP code for AxonASP, include the following checklist in your prompt to ensure compliance with the coding standards outlined above:

```text
STRICT CLASSIC ASP & VBSCRIPT CODING GUIDELINES

Overview: Classic ASP (Active Server Pages) powered by VBScript 5.8 has a highly specific syntax that predates modern programming paradigms. To ensure maximum compatibility, performance, and stability, all ASP code must strictly adhere to the following Microsoft IIS legacy standards. 
