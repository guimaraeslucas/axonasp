            ...
            <form id="uploadForm" action="?step=2" method="post" enctype="multipart/form-data">
                ...
                <fieldset>
                    ...
                    <div class="field-row">
                        ...
                        <label for="modeEnv">Use System Default</label> &nbsp; 
                    ...
                    <div id="manualFields" style="display:none; margin-top:10px; border-top:1px solid #ccc; padding-top:10px;">
                        <div id="rowHost" class="field-row-stacked">
                            ...
                            <input type="text" id="dbHost" name="dbHost" value="localhost" placeholder="<%= obj.AxGetConfig("g3db.mysql_host") %>">
                        </div>
                        <div id="rowPort" class="field-row-stacked">
                            <label for="dbPort">Port:</label>
                            <input type="text" id="dbPort" name="dbPort" value="" placeholder="<%= obj.AxGetConfig("g3db.mysql_port") %>">
                        </div>
                        <div id="rowDB" class="field-row-stacked">
                            <label for="dbName">Database Name / Path:</label>
                            <input type="text" id="dbName" name="dbName" value="imported_data.db" placeholder="<%= obj.AxGetConfig("g3db.mysql_database") %>">
                        </div>
                        <div id="rowUser" class="field-row-stacked">
                            <label for="dbUser">Username:</label>
                            <input type="text" id="dbUser" name="dbUser" value="" placeholder="<%= obj.AxGetConfig("g3db.mysql_user") %>">
                        </div>
...
Case 2 ' Process Upload And List Tables
    ...
    Session("DbType") = uploader.Form("dbType")
    Session("ConnMode") = uploader.Form("connMode")
    Session("DbHost") = uploader.Form("dbHost")
    Session("DbPort") = uploader.Form("dbPort")
...
Dim cat, tbl, Count
Dim srcConn, srcConnStr, sourceConnOk, sourceErr
Dim tableIndex, tableType, tableName
Count = 0
On Error Resume Next
sourceConnOk = False
sourceErr = ""
...
If Not sourceConnOk Then
%>
                    <tr><td style="color:red;">Error opening source database: <%= sourceErr %></td></tr>
<%
Else
    Set cat = Server.CreateObject("ADOX.Catalog")
    Set cat.ActiveConnection = srcConn

    If Err.Number <> 0 Then
%>
                    <tr><td style="color:red;">Error loading database schema: <%= Err.Description %></td></tr>
<%
        Err.Clear
    Else
        For tableIndex = 0 To cat.Tables.Count - 1
            Set tbl = cat.Tables.Item(tableIndex)

            If Err.Number <> 0 Then
                Err.Clear
            Else
                tableType = UCase(Trim("" & tbl.Type))
                If tableType = "TABLE" Or tableType = "LINK" Then
                    tableName = "" & tbl.Name
                    If tableName <> "" Then
            Count = Count + 1
%>
                            <tr>
                                <td>
                                    <input type="checkbox" name="tables" value="<%= Server.HTMLEncode(tableName) %>" checked>
                                    <%= Server.HTMLEncode(tableName) %>
                                </td>
                            </tr>
<%
                    End If
...
%>
                    <tr><td style="color:#333;">No user tables were detected in this Access database.</td></tr>
<%
    End If

    If Not srcConn Is Nothing Then
        If srcConn.State = 1 Then srcConn.Close
    End If
End If
On Error Goto 0
%>
